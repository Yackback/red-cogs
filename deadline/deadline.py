...
dflt_guild = {"edit_mode": False,
              "footer": ("Make sure to subscribe to alerts in the "
                         "#react-for-roles channel so you can get notified when"
                         " we release more breaking news"),
...
class Deadline(object):
  ...
  @deadline_main.command(name="begin")
  async def deadline_begin(self, ctx, url : str, friday_morning: bool = False):
    ...
    :param: friday_morning: whether to check for a friday morning update. To be
    used after a thursday preview. Defaults to False."""
    
    # Quickly set the deadline URL if it is one
    if ("https://" in url or "http://" in url) and "deadline" in url:
        await self.config.deadline_url.set(url)
    else:
    ...
    def reset_edit_mode(self, ctx):
        await self.config.guild(ctx.guild).edit_mode.set(False)
    
    # Quickly set up the Friday morning check if needed
    if friday_morning:
        friday_morning_trigger = CronTrigger(day_of_week="fri", hour="10-11",
                                             minute="*")
        scheduler.add_job(deadline_update, friday_morning_trigger)
        reset_edit_mode_trigger_fri_morn = OrTrigger([CronTrigger(day_of_week="fri",
                                                                  hour="10")])
        reset_edit_mode_fri_morn = scheduler.add_job(reset_edit_mode,
                                                     reset_edit_mode_trigger_fri_morn)
        
    # Trigger Warning ...sorry
    # Set when the triggers go.
    friday_afternoon_trigger = CronTrigger(day_of_week="fri", hour="14-17", minute="*")
    friday_night_trigger = OrTrigger([CronTrigger(day_of_week="fri", hour="22-23", minute="*/2"),
                                      CronTrigger(day_of_week="sat", hour="0-2", minute="*"),
                                      CronTrigger(day_of_week="sat", hour="3", minute="*/2")])
    saturday_morning_trigger = OrTrigger([CronTrigger(day_of_week="sat", hour="8", minute="*/2"),
                                          CronTrigger(day_of_week="sat", hour="9-11", minute="*")])
    saturday_night_trigger = OrTrigger([CronTrigger(day_of_week="sat", hour="22-23", minute="*"),
                                        CronTrigger(day_of_week="sun", hour="0-1", minute="*/2")])
    sunday_morning_trigger = OrTrigger([CronTrigger(day_of_week="sun", hour="8", minute="*/2"),
                                          CronTrigger(day_of_week="sun", hour="9-11", minute="*")])
    ...
    scheduler.add_job(reset_edit_mode)
    
    # Weekends on, time to go to work.
    friday_afternoon = scheduler.add_job(deadline_update, friday_afternoon_trigger)
    friday_night = scheduler.add_job(deadline_update, friday_night_trigger)
    saturday_morning = scheduler.add_job(deadline_update, saturday_morning_trigger)
    saturday_night = scheduler.add_job(deadline_update, saturday_night_trigger)
    sunday_morning = scheduler.add_job(deadline_update, sunday_morning_trigger)
    ...
    async def deadline_update(self, ctx):
        ...
        return None # Just exit the method and return something.
